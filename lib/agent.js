...
export async function runAgent(history, onStep) {
  ...
  // Без штучного ліміту кроків — агент сам вирішує, коли достатньо даних для
  // відповіді (це керується system-промптом: "коли достатньо даних — давай
  // фінал"). SAFETY_CEILING — це НЕ бізнес-ліміт, а аварійний запобіжник від
  // нескінченного циклу/некерованих витрат, якщо модель застрягне в петлі
  // викликів інструментів. У нормальній роботі до нього не повинно доходити.
  const SAFETY_CEILING = 40;
  for (let step = 0; step < SAFETY_CEILING; step++) {
