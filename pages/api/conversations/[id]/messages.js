...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === "POST") {
      ...
      let reply;
      try {
        reply = await runAgent(history);
      } catch (e) {
        return res.status(500).json({ error: `Агент впав: ${e.message}` });
      }
    ...
    // ЗАВЖДИ повертаємо валідний JSON, навіть при неочікуваному падінні
    // (напр. DATABASE_URL не задано/невірний) — щоб фронт не завис на спінері.
